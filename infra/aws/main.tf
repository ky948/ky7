...
resource "aws_ecr_lifecycle_policy" "app" {
  ...
  policy = jsonencode({
    rules = [
      {
        rulePriority = 1
        description  = "Keep newest 20 tagged images"
        selection = {
          tagStatus   = "tagged"
...
resource "aws_secretsmanager_secret" "gemini" {
  name                    = "${local.app_name}/gemini-api-key"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret" "binance_key" {
  name                    = "${local.app_name}/binance-api-key"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret" "binance_secret" {
  name                    = "${local.app_name}/binance-api-secret"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret" "destination_wallet" {
  name                    = "${local.app_name}/destination-wallet"
  recovery_window_in_days = 0
}

resource "aws_secretsmanager_secret" "destination_network" {
  name                    = "${local.app_name}/destination-network"
  recovery_window_in_days = 0
}
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = [
        ...
        aws_secretsmanager_secret.binance_key.arn,
        aws_secretsmanager_secret.binance_secret.arn,
        aws_secretsmanager_secret.destination_wallet.arn,
        aws_secretsmanager_secret.destination_network.arn
      ]
    }]
  })
}
...
resource "aws_ecs_task_definition" "app" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      ...
      { name = "TRADING_MODE", value = "LIVE_VAULT" },
      { name = "LIVE_AUTONOMOUS_ENABLED", value = tostring(var.enable_live_autonomous) },
      { name = "LIVE_PRINCIPAL_USDT", value = tostring(var.live_principal_usdt) },
      { name = "MAX_CAPITAL_ALLOCATION", value = tostring(var.max_capital_allocation) },
      { name = "MAX_POSITION_SIZE", value = tostring(var.max_position_size) },
      { name = "MAX_DAILY_LOSS", value = tostring(var.max_daily_loss) },
      { name = "MAX_DRAWDOWN", value = tostring(var.max_drawdown) },
      { name = "BINANCE_BASE_URL", value = "https://api.binance.com" },
      { name = "BINANCE_RECV_WINDOW", value = "5000" },
      { name = "BINANCE_IP_RESTRICTION_REQUIRED", value = "true" },
      { name = "BINANCE_ENABLE_WITHDRAWALS", value = tostring(var.enable_withdrawals) },
      { name = "PROFIT_SWEEP_ENABLED", value = tostring(var.profit_sweep_enabled) },
      { name = "PROFIT_SWEEP_MINIMUM", value = "50" },
      { name = "PROFIT_SWEEP_PERCENTAGE", value = "50" },
      { name = "PROFIT_SWEEP_ASSET", value = "USDT" },
      { name = "POSTGRES_HOST", value = aws_db_instance.main.address },
      { name = "POSTGRES_PORT", value = "5432" },
      { name = "POSTGRES_DB", value = "ky7" },
      { name = "POSTGRES_USER", value = "ky7" },
...
resource "aws_sns_topic" "alerts" {
  name = "${var.name}-production-alerts"
}
